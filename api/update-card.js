module.exports = async function handler(req, res){
  try{
    ...
    const index = Number(body.index || 0); // 1-based
    const model = typeof body.model === 'string' ? body.model.trim() : '';
    if(!Number.isFinite(index) || index < 1 || !model){ res.statusCode = 400; return res.end('Invalid payload'); }
    ...
    const i = index - 1; if(i < 0 || i >= items.length){ res.statusCode = 400; return res.end('index out of range'); }
    const it = items[i] || {}; it.model = model; items[i] = it;
    const jsonText = JSON.stringify(items, null, 2) + '\n';
    const jsonBase64 = Buffer.from(jsonText,'utf8').toString('base64');
    await ghPut('data/prompts.json', jsonBase64, `chore(api): set model for #${index} -> ${model}`, meta.sha);
