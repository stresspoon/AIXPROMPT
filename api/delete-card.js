module.exports = async function handler(req, res){
  try{
    ...
    const index = Number(body.index || 0); // 1-based index
    const reset = Boolean(body.reset || false);
    if(!reset && (!Number.isFinite(index) || index < 1)){ res.statusCode = 400; return res.end('Invalid index'); }
    ...
    let next = [];
    if(reset){ next = []; }
    else { next = items.filter((_, i)=> i !== (index-1)); }

    const jsonText = JSON.stringify(next, null, 2) + '\n';
    const jsonBase64 = Buffer.from(jsonText,'utf8').toString('base64');
    await ghPut('data/prompts.json', jsonBase64, reset ? 'chore(api): reset prompts' : `chore(api): delete card #${index}`, sha);
